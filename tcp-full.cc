...
#include <map>
std::map<int, int> flow_max_level;
#define DBGTCP 0
#define DBGCOST 0
//(curseq_>1000000)?4:0
...
//	DBGMARK(DBGTCP,2,"***** argc: %d\n",argc);
	...
	if (argc == 2) {
		// Added for flow level tracking
		if (strcmp(argv[1], "get-max-level") == 0) {
			int level = -1;
			if (flow_max_level.find(fid_) != flow_max_level.end()) {
				level = flow_max_level[fid_];
			}
			Tcl::instance().resultf("%d", level);
			return (TCL_OK);
		}
	}
