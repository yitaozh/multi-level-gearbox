#ifndef QUEUE_GEARBOX_FIVE_LEVELS_H
#define QUEUE_GEARBOX_FIVE_LEVELS_H
...
static const int FIFO_PER_LEVEL = 6;
static const int POWERS[NUM_LEVELS] = {1, FIFO_PER_LEVEL, FIFO_PER_LEVEL * FIFO_PER_LEVEL,
                                        ...
                                        FIFO_PER_LEVEL * FIFO_PER_LEVEL * FIFO_PER_LEVEL * FIFO_PER_LEVEL};

// TODO(yitao): not implemented for now
static const int STEP_DOWN_FIFO = 3;
/**
...
class GearboxFiveLevels : public Queue {
private:
    int num_levels_; 
    int current_round_;
    int pkt_count_;
...
public:
    GearboxFiveLevels();
    explicit GearboxFiveLevels(int);
    void enque(Packet*);
    Packet* deque();
